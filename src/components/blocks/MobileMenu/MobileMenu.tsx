...
export default function MobileMenu() {
  const mobileMenuVisibility = useSelector(
    (state: RootState) => state.uiStates.mobileMenu
  );
  ...
  useEffect(() => {
    if (initial) {
      if (mobileMenuVisibility) {
        ...
        gsap.to(mobileMenuRef.current, { left: viewportHeight, duration: 0.5 });
      }
    } else {
  ...
  return (
    <div className="mobile-menu" ref={mobileMenuRef}>
      <div className="mobile-location">
        ...
        <p className="mobile-location__text">Москва</p>
      </div>
      ...
      <ContactInfo
        name="e-mail"
        link={{ text: "test@mail.ru", href: "mailto:test@mail.ru" }}
      ></ContactInfo>
